...
//    }
    ...
    @Override
    @Transactional
    public AverageScopeDTO saveOrUpdateAverageScope(Long movieId, Double scope, Long userId, boolean isUpdate) {
        // 영화 조회
        Movie movie = movieRepository.findById(movieId)
                .orElseThrow(() -> new IllegalArgumentException("해당 ID의 영화가 없습니다."));
        ...
        if (averageScope == null) {
            ...
            if (isUpdate) {
                ...
                if (existingScope == null) {
                    throw new IllegalArgumentException("기존 별점이 존재하지 않습니다.");
                }

                double oldScope = existingScope.getScope(); // 기존 별점 값
                double currentTotal = averageScope.getAverageScope() * currentCount;

                // 새로운 평균 계산 (기존 값 수정)
                newAverage = (currentTotal - oldScope + scope) / currentCount;
            } else {
                // 새로운 별점 추가
                newAverage = ((averageScope.getAverageScope() * currentCount) + scope) / (currentCount + 1);
                averageScope.setNumberOfPeople(currentCount + 1); // 사람 수 증가
            }
